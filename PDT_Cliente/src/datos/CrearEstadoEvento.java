...
public class CrearEstadoEvento {
	
	static Usuario usuario;

	public static boolean crear(String nombre) throws NamingException {
		...
		try {
			...
			estado.setNombre(nombre.toUpperCase());
			
			estado.setEstado(Estado.ACTIVO);
		
			estadoEventoBean.create(estado);	
			
			ListaEventos.cargarLista();
			ListaEstadosEvento.cargarLista();
			PanelListasEvento.construirTablaEstado(ListaEstadosEvento.getListaStringListado(), PanelListasEvento.getTitulosEstado());
			PanelListadoEventos.construirTabla(ListaEventos.getListaStringListado(),PanelListadoEventos.getTitulos());
			PanelListadoEventos.getEstadoComboBox().setModel(new DefaultComboBoxModel<String>(PanelListadoEventos.getEstados()));
			
			JOptionPane.showMessageDialog(new JFrame(), "Estado de Evento creado correctamente");
			
			return true;
			
		} catch (ServicesException e) {
			PanelListasEvento.setAviso("Ya existe un estado de evento con ese nombre");
		}
		return false;
